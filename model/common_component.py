...
class Channel(nn.Module):
    ...
    def complex_normalize(self, x): #imagewise normalize, attention when using masking operation (making return 1d tensor).
        ...
        out = out * torch.sqrt(torch.tensor(int(x.shape[1]), dtype=torch.float32, device=x.device))
...
def get_NROI_attention_mask(ROI_Index,B,H,W):
    Ih,Iw = ROI_Index #Ih: height of interest index, Iw: width of interest index
    ...
    NROI_attention_mask = torch.ones(B,H,W)
    NROI_attention_mask = NROI_attention_mask- total_attention
    
    return NROI_attention_mask
...
def patch_division(image_tensor,d):
    d = int(d)
    image_tensor_dim = image_tensor.size()
    height = image_tensor_dim[2]
    weight = image_tensor_dim[3]    
    
    patch_division_info = [[i for i in range(d)] for j in range(d)]
    patch_size = [height//d,weight//d]
    
    batch_patch_image_tensor = torch.zeros_like(image_tensor)
    batch_patch_image_tensor = batch_patch_image_tensor.reshape(image_tensor_dim[0],int(d*d),image_tensor_dim[1],patch_size[0],patch_size[1])
    

    for h in range(len(patch_division_info)):
        for w in patch_division_info[h]:
            mask = torch.zeros_like(batch_patch_image_tensor)
            mask[:,h*d+w,:,:,:] = 1.
            mask = mask.bool()
            patch = image_tensor[:,:,h*patch_size[0]:(h+1)*patch_size[0],w*patch_size[1]:(w+1)*patch_size[1]]
            batch_patch_image_tensor[mask] = patch.reshape(-1)
...
def reverse_patch_division(batch_patch_image_tensor):
    ...
    image_tensor = torch.zeros_like(batch_patch_image_tensor)
    image_tensor = image_tensor.reshape(B,C,H,W) #image_tensor
    patch_division_info = [[i for i in range(d)] for j in range(d)]
    
    number = 0
    for h_info in range(len(patch_division_info)):
        for w_info in patch_division_info[h_info]:
            mask = torch.zeros_like(image_tensor)
            mask[:,:,h_info*h:(h_info+1)*h,w_info*w:(w_info+1)*w] = 1.
            mask = mask.bool()        
            image_tensor[mask]= batch_patch_image_tensor[:,number,:,:,:].reshape(-1)
            number +=1    
...
def oracle_patch_rate_allocation(patch_wise_psnr, channel_rate_list):
    ...
    patch_wise_psnr = patch_wise_psnr.clone().detach().to(device)
...
def get_mask_info_list(patch_wise_psnr, channel_rate_list):
    #patch_wise_psnr: B X d^2, channel_rate_list: list of possible patchwise channel rate
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    #patch_wise_psnr = torch.tensor(patch_wise_psnr).clone().detach().to(device)
    patch_wise_psnr = patch_wise_psnr.clone().detach().to(device)
    ...
    for rate_num in range(len(channel_rate_list)):
        if rate_num ==0:
            ...
            interest_sort_index = sort_index[:,rate_allocation_standard_list[rate_num-1]:rate_allocation_standard_list[rate_num]]
        mask_info = torch.zeros_like(patch_wise_psnr)
        for i in range(len(interest_sort_index)):
            mask_info[i,interest_sort_index[i]] = 1.
        mask_info_list.append(mask_info)
    
    # mask_info: B X d^2, each element means whether specific patch belongs to group. 
    # mask_info_list: [best_group_mask_info,average_group_mask_info,worst_group_mask_info]
