...
def TestDataMaker(cfg: DictConfig):
    ...
    elif cfg.test_data == "Set5":
        data = Set5dataset_test(cfg)
    elif cfg.test_data == "Set14":
        data = Set14dataset_test(cfg)
    else:
...
class DIV2Kdataset_train():
    def __init__(self,cfg: DictConfig):
        ...
        self.trainset = DatasetFromFolder(
            os.path.join(data_dir, "DIV2K", "DIV2K_train_HR"), transform=self.train_preprocessor
        )
...
class DIV2Kdataset_test():
    def __init__(self,cfg: DictConfig):
        ...
        self.testset = Datasets(os.path.join(data_dir, "DIV2K", "DIV2K_valid_HR"))
...
class Kodakdataset_test():
    def __init__(self,cfg: DictConfig):
        ...
        self.testset = Datasets(os.path.join(data_dir, "Kodak"))
        self.testloader = DataLoader(
            self.testset,
            batch_size=1,
            shuffle=False,
...
class Set5dataset_test():
    def __init__(self, cfg: DictConfig):
        self.test_preprocessor = transforms.Compose([transforms.ToTensor()])
        
        data_dir = cfg.data_dir
        
        self.testset = Datasets(os.path.join(data_dir, "Set5", "HR"))
        self.testloader = DataLoader(
            self.testset,
            batch_size=1,
            shuffle=False,
...
class Set14dataset_test():
    def __init__(self, cfg: DictConfig):
        self.test_preprocessor = transforms.Compose([transforms.ToTensor()])
        
        data_dir = cfg.data_dir
        
        self.testset = Datasets(os.path.join(data_dir, "Set14", "HR"))
        self.testloader = DataLoader(
            self.testset,
            batch_size=1,
            shuffle=False,
...
class Flickr30kdataset_train():
    def __init__(self,cfg: DictConfig):
        ...
        self.trainset = DatasetFromFolder(
            os.path.join(data_dir, "flickr30k", "flickr30k-images"),
...
class DatasetFromFolder(torch.utils.data.Dataset):
    def __init__(self, dataset_dir, transform):
        ...
        self.image_filenames = [
            os.path.join(dataset_dir, x) for x in listdir(dataset_dir) if is_image_file(x)
        ]
        self.transform = transform

    def __getitem__(self, index):
        image = self.transform(Image.open(self.image_filenames[index]).convert('RGB'))
        label = 0
        return image, label
